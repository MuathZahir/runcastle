...
## How the bridge works (no bun patch, no compiler)

1. **Suppress node-pty's install hook.** The root `package.json` declares
   `trustedDependencies` **without** node-pty. Specifying that field replaces
   bun's default trusted list, so node-pty's untrusted `install`/`postinstall`
   scripts never run — the `node-gyp rebuild` fallback that aborts toolchain-less
   installs is never reached.
...
3. **Bridge on postinstall.** The root `postinstall`
   ...
   always run regardless of `trustedDependencies`. The copy is **idempotent** and
   a **no-op on Windows/macOS** (their prebuilds ship in the tarball) and on
   **musl/Alpine** (a glibc prebuild can't load there). It never throws — a
   thrown postinstall would abort `bun install`.
...
> **Why not `bun patch` / `patchedDependencies`?** That was attempt 1 and is a
> dead end: bun cannot create a *new* directory (`prebuilds/linux-x64/`) inside an
> installed package when applying a patch (bun #13770/#22137).
...
the `trustedDependencies` override, the root `postinstall`, and
