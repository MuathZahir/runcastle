...
async function realExecuteTicketRun(
  ...
  const guardOn = config.burnGuard && config.sandbox !== 'noSandbox'
  const guardInstall = guardOn ? buildGuardInstallCommand(model.runtime) : undefined
  ...
  const agentOptions: BurnAgentOptions = guardOn ? { bypassHookTrust: true } : {}
