...
2. `docs[]` inlines the canonical docs in full — `brief.md`, `map.md`, `decisions.md`, `spec.md` if it exists. One too large to inline is listed in `notInlined` instead, with its `relPath` and `absPath`: read it with `mcp__runcastle__read_feature_doc({ relPath })` or off disk before acting. Any tickets arrive as summary rows with their `goal`; `mcp__runcastle__get_ticket({ seq })` holds one ticket's context, acceptance criteria and digest. Read `map.md` (Destination, Notes, Not-yet-specified, Out-of-scope) so your answer fits the north star, and `decisions.md` for what is already locked — **do not re-litigate settled decisions.**
