...
1. **Read the evidence before you plan.** Not after, and not instead of the interview. Your inputs, in reading order:
   - **The previous lap's review evidence.** Your injected prompt names it by absolute path: `<dir>/DIGEST.md` is the review agent's own account of what it found, the same directory holds that pass's screenshots and `walkthrough.webm`, and the review ticket's own `status` in `get_feature_context` is the pass's outcome. It reaches you through nothing else — the directory is host scratch space outside the repo, and every ticket's `digest` is stripped out of the context payload. If the prompt says the previous lap left none, say that plainly; do not imply you read one.
