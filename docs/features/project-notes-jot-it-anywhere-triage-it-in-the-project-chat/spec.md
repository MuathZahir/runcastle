...
**Capture.** On any screen inside a project — project home, a feature workspace, the project chat, preparation — the human presses **⌘/Ctrl+J**, clicks the titlebar **Note** button, or picks **New note** from the ⌘K palette. A small popover floats over the current screen: one focused text line, and a pasted screenshot shows as a thumbnail. **Enter** saves and closes; **Escape** discards. The popover confirms "Saved · N open", the count linking to the inbox. The hotkey works even while focus is inside an embedded terminal. There is no capture on the portfolio home (no project to attach to) and no project picker anywhere — the note belongs to the project you are in.

**See the pile.** The rail's pinned project row carries a badge with the open-note count, hidden at zero. The project workspace's resting page gains a **Notes** card between the New chat card and the conversation list: a **Triage N notes** button; the open notes newest first (text, thumbnail that enlarges on click, inline edit, delete, **Dismiss**); and a collapsed **Triaged (N)** section showing each note's outcome line, its feature link when it has one, and **Reopen**. The card is hidden when the project has never had a note. There is no separate page or route.
