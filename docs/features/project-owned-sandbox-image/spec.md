...
**The button builds the resolved chain and cannot clobber.** Build/Rebuild builds whatever the project resolves to — stock only, or the two-step chain. When `sandboxImage` is a hand-typed (`userSupplied`) tag with no runcastle-managed Dockerfile behind it, the button disarms: the row states the image is custom and managed outside runcastle, with fix text about clearing the setting or adopting `.runcastle/sandbox/Dockerfile`.
