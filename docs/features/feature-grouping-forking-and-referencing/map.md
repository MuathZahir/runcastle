...
**Ideas raised, parked for the waypoint that owns them:** ~~a question-shaped `why(...)` tool returning cited answers~~ (rejected, decision 14); ~~indexing burn outcomes as project knowledge~~ (settled as a facts-only work record, decision 15); recurring burner failures graduating into `CLAUDE.md` rules (decision 23 makes them a health-sweep *input*; the graduation mechanism is still unowned); ~~cross-feature conflict detection on overlapping ticket `seams`~~ (rejected, decision 27 — seams are uncoordinated prose; advisory intake before code, `git merge-tree` shape after); ~~"main moved under you" drift warnings~~ (settled inside decision 27 — same merge-tree check, named not built); shipped features as a free changelog; ~~the `ask-matt` observation that Matt has three on-ramps (grill / triage / diagnose) where runcastle has one~~ (settled, decision 22: one door, five destinations; diagnosis belongs to the burner).
