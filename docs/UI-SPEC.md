...
- **Kickoff delivery (ADR-0009)**: every session is opened with a briefing typed into it (per-kind line, or a per-purpose override like the merge-conflict resolution). Delivery is *confirmed*, not assumed: the `UserPromptSubmit` hook echoing the injected line is the receipt; unconfirmed after 12s it is cleared (`Ctrl-U`) and re-typed, up to 3 attempts. Out of attempts → `session.kickoff_undelivered`; no `SessionStart` within 25s → `session.not_ready`. Both render a warning bar in the session strip with **Send briefing** (`feature.resendKickoff`), which re-types the stored line. A prompt the human typed first settles delivery as `superseded` — runcastle never injects over a live conversation.
