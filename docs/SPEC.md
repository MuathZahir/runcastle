...
- `feature.launchSession({ featureId, kind, kickoffLine? }): { sessionId }` (B1 behavior; `kickoffLine` replaces the per-kind opening briefing for one session). The briefing rides the agent's own argv as its positional initial prompt at spawn (ADR-0009) — a fresh launch only; a resumed one carries no briefing, and an explicit `kickoffLine` is what makes a launch fresh.
