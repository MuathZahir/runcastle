...
export function TicketRow({ ticket, roster, readonly, onEdit, onModel, onCancel, onCopySha }: {
  ...
  return <article className="border-b border-hairline last:border-b-0">
    ...
    {open && !editing && <div className="grid gap-4 border-t border-hairline bg-panel-2 p-4">
      ...
      <div className={section}><div className={heading}>Commits</div>{ticket.commits.length === 0 ? <div className="font-mono text-xs text-text-4">no commits yet — the burn writes them</div> : <div className="flex flex-wrap gap-1.5">{ticket.commits.map((sha) => <button key={sha} type="button" className="rounded-sm border border-hairline bg-transparent px-2 py-1 font-mono text-xs text-text-2" onClick={() => onCopySha(sha)}>{shortSha(sha)}</button>)}</div>}</div>
