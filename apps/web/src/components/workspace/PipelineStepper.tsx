        ...
        <Fragment key={s.phase}>
          <button
            className={[
              ...
              s.clickable ? 'cursor-pointer hover:bg-panel hover:text-text active:scale-[0.97]' : 'cursor-default',
            ]
              .filter(Boolean)
              .join(' ')}
            title={s.tip}
          ...
          >
            <span
              className={`size-2 shrink-0 rounded-pill bg-current transition-[opacity,box-shadow] duration-(--dur-2) ${DOT_STATE[s.state]}`}
            />
            <span>{s.label}</span>
          </button>
          {i < steps.length - 1 && (
