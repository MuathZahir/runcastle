 ...
 * decision 8) — first in `secondary`, except where the resolver promoted it to
 * the primary and rendering it twice would be the bug.
 */
const CHAT_ACTION: NextAction = { label: 'Chat', kind: 'chat' }
...
describe('nextStep — Resume vs Start wording for the chat', () => {
  ...
  it('says Resume once an ended grill session left a resumable conversation', () => {
    ...
    expect(ns.primary).toEqual({ label: 'Resume session', kind: 'chat' })
...
describe('nextStep at building with no tickets', () => {
  ...
  it('points at the thing that produces tickets', () => {
    const ns = nextStep(buildFull(), { driving: false })
    expect(ns.primary).toEqual({ label: 'Open a session', kind: 'chat' })
  ...
  it('goes status-only while a session is live instead of launching a second one', () => {
    ...
    // hide the one click that ships what is on the branch (decisions §3). Chat
    // is the door to the session already live, not a second launch.
...
describe('nextStep after a server restart interrupted a burn', () => {
  ...
  it('offers one Resume burn action with the persistent interruption alert', () => {
    ...
    expect(ns.secondary).toEqual([CHAT_ACTION])
  })

  // `advance` died with the six-phase pipeline (decisions §3): with every
  // ticket landed, resuming the burn is the one road left — it finalizes and
  // the runner's auto-advance crosses to review on its own.
  it('offers to finalize by resuming when the interruption left no pending tickets', () => {
...
describe('nextStep — live sessions go status-only', () => {
  ...
  it('stays status-only while a chat is live, even once decisions.md exists', () => {
  ...
  // human was already typing in still offered to start one. The constant Chat
  // door is not that offer: it is the way back to the terminal already up, and
  // the primary is what must stay silent (decision 8).
  it('never promotes the chat to primary while the terminal is still launching', () => {
    const ns = nextStep(auditFull({ live: true, sessionStatus: 'launching' }), { driving: false })
    expect(ns.kick).toBe('SESSION LIVE')
    expect(ns.primary).toBeUndefined()
    expect(kinds(ns)).toEqual([undefined, 'chat', 'merge'])
  })
  ...
  it('keeps Burn primary while live, once the session has finished the tickets', () => {
    ...
    // Chat no longer waits for the terminal to be free — the door is the same in
    // both bars, and only its hint distinguishes them (decision 8).
  ...
  it('reads the shape of the tickets it is about to burn, without disarming Burn', () => {
    const base = auditFull({ tickets: 2 })
  ...
  it('names the docs digest this road will hand every ticket, too', () => {
    const ns = nextStep(auditFull({ tickets: 2 }), {
      driving: false,
  ...
  it('still offers a chat to emit the first tickets when nothing is live', () => {
    const ns = nextStep(auditFull({ docs: ['decisions.md', 'spec.md'] }), { driving: false })
    expect(ns.primary).toEqual({ label: 'Start session', kind: 'chat' })
  })
...
describe('nextStep at review', () => {
  const reviewFull = (opts: {
    ...
    sessionLive?: boolean
    lap?: number
    /** Which lap each ticket belongs to, when the pending work spans laps. */
    ticketLaps?: number[]
  ...
  it('promotes Burn to primary and drops Merge & ship to secondary with a pending ticket', () => {
    ...
    expect(labels(ns.secondary)).toEqual(['Chat', 'Merge & ship', 'Start test drive', 'Iterate'])
    expect(ns.secondary).toContainEqual({ label: 'Iterate', kind: 'iterate' })
  })
  ...
  it('compounds Iterate with the end of a live session, never disabling it', () => {
    ...
    expect(labels(ns.secondary)).toEqual(['Chat', 'Start test drive', 'End session & iterate'])
  ...
  describe('from the server’s drive state', () => {
    ...
    it('reads a bare checkout as inspection, with Stop primary and Merge one click below', () => {
      ...
      expect(labels(ns.secondary)).toEqual(['Chat', 'Merge & ship', 'Iterate'])
    })
    ...
    it('offers Fix drive over a failed setup, with the stop beside it', () => {
      ...
      expect(labels(ns.secondary)).toEqual(['Chat', 'Merge & ship', 'Stop test drive', 'Iterate'])
    })
  ...
  it('still promotes Burn when a session is live with pending tickets', () => {
    const ns = nextStep(
  ...
  /**
   * One chat per feature, decision 8 — review is the page that had NO session
   * door: asking the agent a question from here meant clicking Rethink, which
   * bumped the lap and threw the feature back to planning. Chat is on every
   * branch this resolver can return, in the same leading position, and it never
   * disturbs the trail's own verbs.
   */
  it('carries the Chat door first on every review branch, whatever else is offered', () => {
    const bars = [
      nextStep(reviewFull({}), { driving: false }),
      nextStep(reviewFull({ ticketStatuses: ['pending'] }), { driving: false }),
      nextStep(reviewFull({}), { driving: false, openNotes: 2 }),
      nextStep(reviewFull({}), { driving: false, driveState: 'setup-failed' }),
      nextStep(reviewFull({}), { driving: false, laterLaps: '- the inspector' }),
      nextStep(reviewFull({ sessionLive: true }), { driving: false }),
      nextStep(reviewFull({}), { driving: true }),
    ]
    for (const ns of bars) {
      expect(ns.secondary[0]).toEqual(CHAT_ACTION)
      expect(ns.secondary.filter((a) => a.kind === 'chat')).toHaveLength(1)
    }
  })
  ...
  /**
   * Ticket 10 / decision 21 — the bar's forward choices collapse to two:
   * Merge & ship, or Iterate. "Address notes" and "Fix N open defects" were the
   * same decision ("this isn't ready") entered through two more doors, with the
   * fork duplicated inside one of them; the triage step behind Iterate is where
   * that choice is made now, once, over the whole list.
   */
  describe('the one door forward', () => {
    it('offers exactly Merge & ship and Iterate, and neither retired verb', () => {
      for (const ctx of [
        { driving: false },
        { driving: false, openNotes: 3 },
        { driving: false, openDefects: 2 },
        { driving: false, openNotes: 1, openDefects: 1 },
      ]) {
        const ns = nextStep(reviewFull({}), ctx)
        const kinds = [ns.primary, ...ns.secondary].map((a) => a?.kind)
        expect(kinds).toContain('merge')
        expect(labels([ns.primary!, ...ns.secondary])).toContain('Iterate')
        expect(kinds).not.toContain('addressNotes')
        expect(kinds).not.toContain('fixDefects')
        expect(labels(ns.secondary)).not.toContain('Address notes')
      }
    })

    // Nothing open to triage: the step would be an empty list, so it is skipped
    // and the click is the lap itself.
    it('goes straight to the lap when nothing is open', () => {
      for (const ctx of [
        { driving: false },
        { driving: false, openNotes: 0, openDefects: 0 },
      ]) {
        const ns = nextStep(reviewFull({}), ctx)
        expect(ns.secondary).toContainEqual({ label: 'Iterate', kind: 'iterate' })
      }
    ...
    it('opens the triage step when notes or defects stand', () => {
      for (const ctx of [
        { driving: false, openNotes: 3 },
        { driving: false, openDefects: 1 },
        { driving: false, openNotes: 1, openDefects: 2 },
      ]) {
        expect(nextStep(reviewFull({}), ctx).primary).toEqual({ label: 'Iterate', kind: 'iterate' })
      }
    })
    ...
    it('makes answering the open work the primary, with Merge one click below', () => {
      ...
      expect(labels(ns.secondary)).toEqual(['Chat', 'Merge & ship', 'Start test drive'])
    ...
    // The drive holds the branch the lap worktree needs, and the server refuses
    // the triage commit for the same reason — so the door itself waits, with the
    // one click that clears it (decision 20).
    it('keeps the drive’s refusal and its escape on the door', () => {
      const ns = nextStep(reviewFull({}), { driving: true, openNotes: 2 })
  ...
  describe('with scope the spec deferred to a later lap', () => {
    ...
    it('flips the primary to starting the next lap, demoting Merge & ship', () => {
      ...
      expect(ns.primary).toEqual({ label: 'Start lap 2', kind: 'iterate' })
    ...
    it('keeps the flip while driving, carrying Iterate’s own reason', () => {
      ...
      expect(labels(ns.secondary)).toEqual(['Chat', 'Merge & ship', 'Stop test drive'])
    })
    ...
    // Open work outranks it: the deferred scope is next lap's, and what is open
    // is this one's — the door in front of the human triages both at once.
  ...
  /**
   * Decision 28a — burning takes every pending ticket on the branch, across
   * laps, which is correct (ADR-0010 accumulates them); the count alone never
   * said whose they were.
   */
  describe('the Burn count says whose tickets it burns', () => {
    it('names the carried laps when the pending work spans them', () => {
      const ns = nextStep(
        reviewFull({
          lap: 2,
          ticketStatuses: ['pending', 'pending', 'pending'],
          ticketLaps: [2, 1, 1],
        }),
        { driving: false },
      )
      expect(ns.primary?.label).toBe('Burn 3 tickets — 1 from lap 2 · 2 carried from lap 1')
    })

    it('stays a plain count when every pending ticket is this lap’s', () => {
      const ns = nextStep(reviewFull({ ticketStatuses: ['pending', 'pending'] }), {
        driving: false,
      })
...
describe('nextStep at planning — the step derived from the artifacts', () => {
  ...
  const pending = [{ id: 't1', seq: 1, status: 'pending', lap: 1, ...BURNABLE }]
...
describe('archive derivations', () => {
  ...
  it('needsMe is null for archived features', () => {
    expect(needsMe(listItem({ status: 'archived', phase: 'planning' }))).toBeNull()
  })

  it('excludes archived features from the default lanes', () => {
    const groups = triage([
      listItem({ id: 'a', status: 'active', phase: 'planning' }),
...
describe.skip('draft derivations', () => {
  ...
  it('wears the ◌ glyph rather than a phase glyph', () => {
    ...
    expect(DRAFT_GLYPH).not.toBe(phaseGlyph('planning'))
  })
  ...
  it('sorts drafts below active work and above shipped', () => {
    const sorted = sortForSidebar([
      ...
      draft({ id: 'draft' }),
      listItem({ id: 'active', phase: 'planning' }),
      listItem({ id: 'needsMe', phase: 'planning' }),
    ])
  ...
  it('nextStep offers Start as a draft’s one action', () => {
    const ns = nextStep(full({ status: 'draft', phase: 'planning' }), { driving: false })
  ...
  it('disables Start until the branch list resolves the base it would send', () => {
    const ns = nextStep(full({ status: 'draft', phase: 'planning' }), {
      driving: false,
...
describe('nextStep at building', () => {
  ...
  /**
   * One chat per feature, decision 8 — the old Revisit door vanished while a
   * session was live, because a second launch would have been refused. The one
   * Chat door is the way back INTO that session, so it is the same bar either
   * way: nothing here is hidden or disabled because a terminal is up.
   */
  it('keeps the Chat door whether or not a session is live', () => {
    const live = nextStep(buildFull({ sessionLive: true }), { driving: false })
    expect(live.secondary).toEqual([CHAT_ACTION, MERGE_ACTION])
  ...
  it('offers Merge & ship throughout, including while the burn runs', () => {
    ...
    expect(running.secondary).toEqual([CHAT_ACTION, MERGE_ACTION])
 ...
 * quiet one must stay the plain hero. This is the derivation the body renders
 * through.
 */
describe('shippedChatSessions', () => {
  const sessions = (rows: unknown[]) => rows as FeatureFull['sessions']

  it('shows a live chat session', () => {
    const rows = sessions([{ id: 's1', status: 'live', kind: 'chat', ccSessionId: null }])
    expect(shippedChatSessions(rows)).toEqual(rows)
  })

  it('shows an ended chat session whose conversation is still on disk', () => {
    const rows = sessions([{ id: 's1', status: 'ended', kind: 'chat', ccSessionId: 'cc-1' }])
    expect(shippedChatSessions(rows)).toEqual(rows)
  })

  it('shows nothing for an ended chat session that never reached live (no cc id)', () => {
    const rows = sessions([{ id: 's1', status: 'ended', kind: 'chat', ccSessionId: null }])
    expect(shippedChatSessions(rows)).toEqual([])
  })

  it('shows nothing on a shipped feature that was never talked to', () => {
 ...
 */
describe.skip('rowChip', () => {
  const NOW = 1_000_000_000_000

  it('shows Needs you, carrying the specific reason as its title', () => {
    const chip = rowChip(listItem({ phase: 'planning' }), NOW)
  ...
  it('colours a failed run’s chip by its own flavour, not the generic amber', () => {
    ...
    const chip = rowChip(listItem({ phase: 'building', ticketCounts: counts }), NOW)
    expect(chip.kind).toBe('needsMe')
  ...
  it('spins while the agent is mid-turn in a live session', () => {
    const chip = rowChip(withSession(false, { phase: 'planning' }), NOW)
    expect(chip.kind).toBe('working')
    expect(chip.text).toBe('Working')
  })

  it('shows Needs you the moment that agent stops and waits', () => {
    const chip = rowChip(withSession(true, { phase: 'planning' }), NOW)
    expect(chip.kind).toBe('needsMe')
    expect(chip.text).toBe('Needs you')
  })
})
 ...
 */
describe.skip('turn-aware feature states', () => {
  it('keeps a mid-turn grill out of Needs you, where the phase alone would put it', () => {
    const f = withSession(false, { phase: 'planning' })
  ...
  it('moves it to Needs you once the agent stops and waits for an answer', () => {
    const f = withSession(true, { phase: 'planning' })
  ...
  it('holds a working session out of Needs you at a phase that would ask for it', () => {
    ...
    const f = withSession(false, { phase: 'planning', ticketCounts: counts })
    expect(needsMe(f)).toBeNull()
    expect(triageOf(f)).toBe('agentWorking')
  })
  ...
  it('leaves a feature with no live session exactly as it was', () => {
    const counts = { total: 2, pending: 2, burning: 0, done: 0, failed: 0, cancelled: 0 }
    expect(needsMe(listItem({ phase: 'planning' }))?.kind).toBe('grill')
    expect(needsMe(listItem({ phase: 'planning', ticketCounts: counts }))?.kind).toBe('burn')
    expect(needsMe(listItem({ phase: 'review' }))?.kind).toBe('ship')
    expect(needsMe(listItem({ phase: 'planning' }))).toBeNull()
    expect(triageOf(listItem({ phase: 'planning' }))).toBe('needsYou')
    expect(triageOf(listItem({ phase: 'planning' }))).toBe('inProgress')
  })
  ...
  it('sorts a waiting session to the top of the rail and a working one below it', () => {
    const sorted = sortForSidebar([
      listItem({ id: 'quiet', phase: 'planning' }),
      withSession(false, { id: 'working', phase: 'planning' }),
      withSession(true, { id: 'waiting', phase: 'planning' }),
    ])
 ...
 */
describe.skip('landingFeature', () => {
  const working = listItem({ id: 'working', activeRun: true })
  const needsYou = listItem({ id: 'needs', phase: 'planning' })
  const inProgress = listItem({ id: 'planning', phase: 'planning' })
  ...
  it('falls to In progress when no agent is working either', () => {
    expect(landingFeature([shipped, draft, inProgress])?.id).toBe('planning')
  })

  it('keeps the incoming order within a lane', () => {
    const first = listItem({ id: 'first', phase: 'planning' })
    const second = listItem({ id: 'second', phase: 'planning' })
