 ...
 * that Agentic review reaches `feature.agenticReview` with this feature, and
 * that the refusal it hands back lands INSIDE the banner rather than in a toast.
 *
...
let reviewCallbacks: MutationCallbacks = {}
    ...
    feature: {
      agenticReview: {
        useMutation: (callbacks: MutationCallbacks) => {
          reviewCallbacks = callbacks
          return { mutate: (...a: unknown[]) => reviewMutate(...a), isPending: false }
        },
      },
    },
  },
...
describe('the denied-drive banner’s action', () => {
  /** Decision 7: a fresh pass on this lap, never a reset of the refused one. */
  it('mints another review pass for the feature the denial belongs to', () => {
    mount()
    fireEvent.click(screen.getByRole('button', { name: 'Agentic review' }))
    expect(reviewMutate).toHaveBeenCalledWith({ featureId: 'ftr_1' })
  })

  it('shows the refusal in the banner, beside the files it is about', () => {
    mount()
    fireEvent.click(screen.getByRole('button', { name: 'Agentic review' }))
    act(() => reviewCallbacks.onError?.({ message: STILL_DIRTY }))
  ...
  it('drops a stale refusal when the human tries again', () => {
    mount()
    const again = screen.getByRole('button', { name: 'Agentic review' })
